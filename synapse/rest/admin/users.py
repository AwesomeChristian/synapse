...
class UserRestServletv2(RestServlet):
    ...
    returns:
        201 OK with empty object if user was created or
        200 OK with empty object if user was modified
        otherwise an error.
    """
    ...
    async def on_GET(self, request, user_id):
        ...
        ret = await self.handlers.admin_handler.get_user(target_user)
        return 200, ret
    ...
    async def on_PUT(self, request, user_id):
        ...
        elif len(target_user) > 512:
            raise SynapseError(400, "Invalid username")

        user = await self.handlers.admin_handler.has_user(target_user)

        if user:  # modify user
            requester = await self.auth.get_user_by_req(request)

            # TODO: allow to change display name
